...
function InversionScene() {
  ...
  return (
    <SceneShell
      ...
      title={<>Naming a country <em className="font-display italic text-red-300">narrows</em> the output below the unspecified prompt.</>}
    >
      <Reveal>
      ...
      <Reveal delay={0.08}>
        <Panel className="mt-6">
          ...
          <p className="mt-3 max-w-2xl text-sm leading-6 text-foreground/60">
            ...
            something different. A long bar means the model generated nearly the same picture 50 times. The unspecified prompt is
          ...
          <div className="mt-6 grid gap-4 border-t border-border pt-5 md:grid-cols-2">
            ...
            <p className="text-sm leading-6 text-foreground/60">
              ...
              {sit === 'funeral'
                ? `The honest exception: the unspecified funeral prompt is already narrow (${defIntra.mean.toFixed(2)}), since a Western church interior is itself a stereotype, so the country qualifier cannot narrow it much further. The comparison is always against the default prompt, never against zero.`
    ...
    <div>
            ...
            <div className="relative h-4 flex-1">
              {[-0.1, 0, 0.1, 0.2, 0.3].map((v) => (
                <span
                  ...
                  style={{ left: pos(v) }}
                >
                  {v === 0 ? 'unspecified prompt' : `${v > 0 ? '+' : ''}${v.toFixed(1)}`}
                </span>
              ))}
            </div>
            <span className="w-14 shrink-0" />
      ...
      <div className="mt-2 flex items-center gap-3">
        ...
        <div className="flex flex-1 justify-between font-mono2 text-[10px] text-foreground/35">
          <span>← more varied than the unspecified prompt</span>
          <span>less varied than the unspecified prompt →</span>
        </div>
        <span className="w-14 shrink-0" />
      </div>
      ...
      <div className="mt-4 flex min-h-[18px] flex-wrap items-center gap-5 border-t border-border pt-3 font-mono2 text-[10px] text-foreground/50">
        {hover ? (
          <span className="text-foreground/80">
            {hover.label}: {signed(hover.d)} against its own unspecified prompt
          </span>
        ) : (
          <>
            <span className="flex items-center gap-1.5"><span className="h-4 w-px bg-foreground/40" /> the unspecified prompt, at zero in every row</span>
    ...
    >
      <Reveal>
        <p className="prose-scene max-w-2xl">
          ...
          land on top of the unspecified prompt. Only the country qualifier moves the set.
        </p>
        <Sd21Only />
      </Reveal>
      ...
      <Reveal delay={0.08}>
        <Panel className="mt-10">
          ...
          <p className="mt-3 max-w-3xl text-sm leading-6 text-foreground/60">
            Every mark is plotted as the <em>change</em> from its own row's unspecified prompt, so the vertical line means
            the same thing in all eight rows: no change in variety. Marks to the right of it are less varied than the
            unspecified prompt, marks to the left are more varied.
          </p>
          <div className="mt-6">
            <SpecificityRows />
          </div>
              ...
              return (
                <div key={ctrl} className="rounded-lg border border-border p-2.5">
                  ...
                  <div className="mt-1.5 font-mono2 text-[9px] text-foreground/40">
                    stays at the unspecified prompt's variety ({lvl?.intraset?.toFixed(2)})
                  </div>
                </div>
              )
            })}
